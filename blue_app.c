...
static void waitOTA_task(void *arg)
{
    ...
    while (1)
    {
        if (BleCmd.OtaCmd == READYOTA)
        {
            ...
            while (FileSize.ReceivingProgress != 100)
            {
...
static void RxData_Process(esp_ble_gatts_cb_param_t *param)
{
    ...
    if (Rxdata.startFlag)
    {
        Rxdata.index++; //更新索引
        /* 接收的校验和 */
        Rxdata.sumData = Rxbuf[Rxdata.startPosition + Rxdata.index] << 24 | Rxbuf[Rxdata.startPosition + Rxdata.index + 1] << 16 | Rxbuf[Rxdata.startPosition + Rxdata.index + 2] << 8 | Rxbuf[Rxdata.startPosition + Rxdata.index + 3];
        ...
        /* 记录接收的ota文件大小 */
        Rxdata.ota_fileSize = Rxbuf[Rxdata.startPosition + Rxdata.index] << 24 | Rxbuf[Rxdata.startPosition + Rxdata.index + 1] << 16 | Rxbuf[Rxdata.startPosition + Rxdata.index + 2] << 8 | Rxbuf[Rxdata.startPosition + Rxdata.index + 3];
